    ...
    try {
        ...
        if (!rawToken) {
            auditService.logSecurityEvent({
                ...
                result: 'DENIED',
                details: { reason: 'Missing token header' }
            });
            return res.status(401).json({
                error: 'IoT device authentication required. Missing bearer token.',
                lcdLine1: 'Access Denied',
                lcdLine2: 'Missing Token'
            });
        }
        ...
        if (devices.length === 0) {
            auditService.logSecurityEvent({
                ...
                resourceType: 'IOT_DEVICE',
                result: 'DENIED',
                details: { reason: 'Unrecognized credential' }
            });
            return res.status(401).json({
                error: 'Invalid or unrecognized IoT device credential.',
                lcdLine1: 'Access Denied',
                lcdLine2: 'Invalid Device'
            });
        }
